...
@Component({
  ...
  styleUrl: './active-suppliers.component.scss',
})
export class ActiveSuppliersComponent implements OnInit {
  ...
  fetchAllActiveSuppliers(page: number, pageSize: number, name: string) {
    this.loading = true;
    this.noSuppliersMessage = null;
    this.apiService.getAllActiveSuppliers(page, pageSize, name).subscribe({
  ...
  deactivatedSupplier(id: number) {
    if (confirm('هل أنت متأكد من حظر هذه المورد')) {
      this.loading = true;
      this.apiService.deactivateSupplier(id).subscribe({
        next: (response) => {
          this.suppliersMessage = 'تم حظر المورد بنجاح';
          setTimeout(() => {
        ...
        error: (error) => {
          ...
          this.noSuppliersMessage = 'فشل حظر المورد';
          this.loading = false;
          setTimeout(() => {
            this.noSuppliersMessage = null;
          }, 2000);
